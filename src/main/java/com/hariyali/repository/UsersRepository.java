...
public interface UsersRepository extends JpaRepository<Users, Integer> {
	...
	@Query(value = "SELECT u.user_id, u.webId, IFNULL(ur.donorId, u.donorId) AS donorId, u.first_name, u.last_name, u.donor_type, u.organisation, u.approval_status, u.emailId, u.remark, SUM(CASE WHEN d.approval_status = 'Pending' and p.paymentInfo_id is not null THEN 1 ELSE 0 END) AS pending_count, CASE WHEN IFNULL(u.aadhar_card, '') = '' then u.pan_card ELSE  u.aadhar_card END AS PANORAADHAR, MIN(d.donation_date) AS DonationDate\n" +
			" ,(SELECT 1 from tbl_user_document where userId = u.user_id AND doc_type<>'CERTIFICATE' limit 1) AS HasDocument " +
			" FROM tbl_user_master u inner join tbl_donation d ON d.userId = u.user_id left join tbl_payment_info p ON d.donation_id = p.donationId LEFT JOIN tbl_recipient r ON r.donationId = d.donation_id LEFT JOIN tbl_user_master ur ON ur.emailId = r.emailID and ur.role_id = 2\n" +
			" WHERE u.approval_status = 'Approved' \n" +
			" AND ((:donorType is not null AND u.donor_type = :donorType) OR :donorType is null)  AND u.role_id = 2 \n" +
			" AND ((:fromDate is not null AND DATE(d.donation_date) between :fromDate and IFNULL(:toDate, SYSDATE() )) OR :fromDate is null)"+
			" AND (u.WebId like CONCAT(:searchText, '%') OR IFNULL(ur.donorId, u.donorId) LIKE CONCAT(:searchText, '%') OR u.emailId LIKE CONCAT(:searchText, '%') \n" +
			" OR concat(u.first_name, ' ', u.last_name) LIKE CONCAT(:searchText, '%') OR u.pan_card LIKE CONCAT(:searchText, '%') OR u.aadhar_card LIKE CONCAT(:searchText, '%') OR u.organisation LIKE CONCAT(:searchText, '%')) \n" +
	...
	@Query(value="select u.donorId from tbl_user_master as u where u.user_id=?;",nativeQuery = true)
	String getDonorIdbyUserId(int userId);
...
//	Users findByEmailIdAndApprovalStatus(String email, String approvalStatus);
	...
	@Query(value = "select ifnull(ur.donorId, u.donorId) as donorId, d.donation_code, d.order_id AS transactionNo, u.donor_type, concat(u.first_name, ' ', u.last_name) as donor_name \n" +
			...
			" from tbl_user_master u INNER JOIN tbl_donation d ON u.user_id = d.userId \n" +
			"     INNER JOIN tbl_payment_info p ON p.donationId = d.donation_id INNER JOIN tbl_user_packages up ON up.donationId = d.donation_id\n" +
			"     LEFT JOIN tbl_recipient r ON d.donation_id = r.donationId \n" +
			"     LEFT JOIN tbl_user_master ur ON r.emailID = ur.emailId and ur.role_id = 2 \n" +
			"WHERE  u.approval_status = 'Approved' AND U.role_id = 2 AND u.user_id = d.userId AND p.donationId = d.donation_id AND up.donationId = d.donation_id AND DATE(d.donation_date) between :fromDate AND :toDate AND ((:donorType is not null AND u.donor_type = :donorType) OR :donorType is null)"+
			" AND (u.WebId like CONCAT(:searchText, '%') OR ifnull(ur.donorId, u.donorId) LIKE CONCAT(:searchText, '%') OR u.emailId LIKE CONCAT(:searchText, '%') \n" +
			" OR concat(u.first_name, ' ', u.last_name) LIKE CONCAT(:searchText, '%') OR u.pan_card LIKE CONCAT(:searchText, '%') OR u.aadhar_card LIKE CONCAT(:searchText, '%') OR u.organisation LIKE CONCAT(:searchText, '%')) \n"
			, nativeQuery = true)
